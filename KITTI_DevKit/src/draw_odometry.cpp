...
void plotPathPlot (string full_name, vector<int32_t> &roi) {
    // gnuplot file name
    char command[1024];
    ...
    fprintf(fp, "set output \"odometry.png\"\n");
    fprintf(fp, "set size ratio -1\n");
    fprintf(fp, "set xrange [%d:%d]\n", roi[0], roi[1]);
    fprintf(fp, "set yrange [%d:%d]\n", roi[2], roi[3]);
    fprintf(fp, "set xlabel \"x [m]\"\n");
    fprintf(fp, "set ylabel \"y [m]\"\n");
    fprintf(fp, "plot \"odometry_pose_xy.txt\" using 1:2 lc rgb \"#000000\" lw 2 title 'Ground Truth' w lines,");
    fprintf(fp, "\"< head -1 odometry_pose_xy.txt\" using 1:2 lc rgb \"#000000\" pt 4 ps 1 lw 2 title 'Sequence Start' w points\n");

    // close file
    fclose(fp);

    // run gnuplot => create png + eps
    sprintf(command, "gnuplot %s; rm odometry_pose_xy.*", full_name.c_str());
    system(command);
}
...
int main (int argc,char *argv[])
{
    // we need 3 arguments
    if (argc != 2) {
        cout << "Usage: ./draw_odometry odometry_path" << endl;
        return -1;
    }

    string odometryFile(argv[1]);
    vector<Matrix> poses = loadPoses(odometryFile);
    double length = trajectoryDistances(poses).back();
    printf("Tatal distance: %fm\n", length);

    // check for errors
    if (poses.size() == 0) {
        cerr << "Couldn't read (all) odometry poses." << endl;
        return -1;
    }

    savePathPlot(poses, "./odometry_pose_xy.txt");
    vector<int32_t> roi = computeRoi(poses);
    plotPathPlot("./odometry_pose_xy.gp", roi);
